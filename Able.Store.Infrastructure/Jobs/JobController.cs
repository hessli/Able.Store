...
namespace Able.Store.Infrastructure.Jobs
{
    public static class JobController
    {
        ...
        public static void AddJob(IJob job, double interval,double delay)
        {
            jobs.Add(new JobOption
            {
                Interval = interval,
                Job = job,
                NextRun=DateTime.Now.AddMilliseconds(delay)
            });
        }
        ...
        public static void Stop(IJob job)
        {
            jobs.Remove(job);
